...
var BATCH_DESCRIPTORS = ['plastic cup', 'cardboard box', 'wood plank', 'daisy flower'];
...
var makeVol = function(url) {
      ...
      for(var i = 0; i < BATCH_DESCRIPTORS.length; i++){
        vol.w[i] = BATCH_DESCRIPTORS[i];
      }
...
var train = function(batch, size) {
  ...
  batch.forEach(function(image){
    ...
    console.log('Training: '+image.descriptor+" on url: "+image.url);
    trainer.train(vol, image.descriptor);
  })
};
...
var trainDescriptors = function(){
    BATCH_DESCRIPTORS.forEach(function(descriptor){
    console.log('Training for: ' +descriptor+' recognition.')
    makeBatch(descriptor, true, 100)
  }); 
}

//pulls in photos from flickr api and can train automatically
var makeBatch = function(descriptor, trainNow, size){
  size = size || 25;
  var batch = [];
  flickr.photos.search({
    ...
    if(err){
      ...
      results.photos.photo.forEach(function(photo){
        var photo_obj = {url: 'https://farm'+photo.farm+'.staticflickr.com/'+photo.server+'/'+photo.id+'_'+photo.secret+'.jpg', descriptor: descriptor};
